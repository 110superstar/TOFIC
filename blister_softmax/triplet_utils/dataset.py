...
class query_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cls_num = index // len(self.query_index)
        idx_num = self.query_index[index % len(self.query_index)]
        image_index = self.label_to_indices[cls_num]
        return self.transform(Image.open(self.train_paths[image_index[idx_num]])), cls_num + self.add_class, idx_num
...
class WrongTripletBlister(Dataset):
    def __init__(self, blister_dataset, class_num, wrong_idx, selected_idx_184, selected_idx_46):
        self.blister_dataset = blister_dataset
        self.transform = self.blister_dataset.transform
        ...
        self.selected_idx_184 = selected_idx_184
        self.selected_idx_46 = selected_idx_46
    ...
    def __getitem__(self, index):
        ...
        label1 = int(self.wrong_idx[index][0])
        img_index = self.label_to_indices[label1][int(self.wrong_idx[index][1])]
        img1 = Image.open(self.train_paths[img_index])
        # 取 positive (確保不會取到 anchor)
        positive_index = int(self.wrong_idx[index][1])
        while positive_index == int(self.wrong_idx[index][1]):
            positive_index = random.choice(self.selected_idx_46)
        positive_img_index = self.label_to_indices[label1][positive_index]
        img2 = Image.open(self.train_paths[positive_img_index])
        # 取 negative
        negative_label = int(self.wrong_idx[index][2]) 
        if negative_label < 184: 
            negative_index = random.choice(self.selected_idx_184) # 利用標記取到影像
        else:
            negative_index = random.choice(self.selected_idx_46) # 利用標記取到影像
